...
def run_detection_A(video_path, durasi):
...
def run_detection_B(video_path, durasi):
    global density, condition, vehicle_counter, green_light_duration
    density1, condition1, vehicle_counter1, green_light_duration1 = detect_objects(video_path, durasi)
    density2, condition2, vehicle_counter2, green_light_duration2 = detect_objects(video_path, durasi)
    ...
    while running:
        ...
        if type(jalan1) == int and jalan1 > 0 and type(jalan4) == str:
            ...
            if perform_detection and type(jalan2) == str:
                ...
                detection_thread = threading.Thread(target=run_detection_A, args=(video_path[index_path], durasi))
            ...
            if kondisi == "Emergency" and perform_detection2:
                durasi = jalan2
                path = index_path + 1
                detection_thread2 = threading.Thread(target=run_detection_B, args=(video_path[path], durasi))
                detection_thread2.start()
                perform_detection2 = False
