...
def deteksi_thread(video_path, durasi):
        ...
        while running:
            ...
            if type(jalan1) == int and jalan1 > 0 and type(jalan4) == str:
                if not cek and type(jalan2) == str:
                    ...
                    process = multiprocessing.Process(target=detect_objects, args=(video_path, durasi))
                    process.start()
    
                    # Retrieve the results from the Pipe
                    density, condition, vehicle_counter, green_light_duration = process.return_value
